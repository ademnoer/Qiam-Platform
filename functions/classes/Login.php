...
class Login {
    ...
    public function updateUserMark() {
        $user_mark = $this->getUserInformation()->user_mark;
        $user_mark = $user_mark + 50;
        $user_name = $_SESSION['user_name'];
        $query_update = $this->db_connection->prepare('UPDATE users SET user_mark = :user_mark WHERE user_name = :user_name');
        $query_update->bindValue(':user_mark', $user_mark, PDO::PARAM_STR);
        $query_update->bindValue(':user_name', $user_name, PDO::PARAM_INT);
    ...
    private function deleteRememberMeCookie() {
        global $user_id;
        // if database connection opened
        if ($this->databaseConnection() && isset($user_id)) {
